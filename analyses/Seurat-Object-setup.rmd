---
title: "Seurat - Guided Clustering Tutorial & Data Visualization"
output:
  html_document:
    theme: united
    df_print: kable
...
#Data visualization method in Seurat
```{r}
...
pbmc3k.final
```
#Five visualization of marker feature expression
```{r}
#Ridge plots from ggridges
RidgePlot(pbmc3k.final, features = features, ncol = 3)
```


```{r}
#Violin plot 
VlnPlot (pbmc3k.final, features = features, ncol = 3)
```

```{r}
#Violin plots split by variables (ex groups)
VlnPlot(pbmc3k.final, features = "percent.mt", split.by = "groups")
```

```{r}
#Feature plot - expression visuazlization in low-dimension space
FeaturePlot (pbmc3k.final, features = features, ncol = 3)
```

```{r}
#New Feature plot
FeaturePlot(pbmc3k.final, features = "MS4A1") #Can do that with cTECs markers 
```

```{r}
#Feature plot with adjusted contrast
FeaturePlot(pbmc3k.final, features = "MS4A1", min.cutoff = 1, max.cutoff = 3)
```
```{r}
#Feature plot with specific conrast with multiple markers plot
FeaturePlot(pbmc3k.final, features = c("MS4A1", "PTPRCAP"), min.cutoff = "q10", max.cutoff = "q90") #determined based on 10th and 90th percentiles of the gene’s expression
```
```{r}
#Feature plot co-expression of 2 features 
FeaturePlot(pbmc3k.final, features = c("MS4A1", "CD79A"), blend = TRUE)
```
```{r}
#Split Feature plor to view expression by group
FeaturePlot(pbmc3k.final, features = c("MS4A1", "CD79A"), split.by = "groups")
```

```{r}
#Dot plots = size of dots corresponds to percentage of cells expresing feature in each cluster --> color = average expression 
DotPlot(pbmc3k.final, features = features) + RotatedAxis() #adjust spacing for label in x axis
```
```{r}
#dot plot also can be splited to groups
DotPlot(pbmc3k.final, features = features, split.by = "groups") + RotatedAxis()
```
```{r}
#Dim plot allow plotting of either umap, tsne or pca
#remotes::install_github('sjessa/ggmin')
DimPlot(pbmc3k.final, reduction = "umap") + labs(title = "Clustering of 2.700 PBMCs") + ggmin::theme_powerpoint() +
```
```{r}
pbmc3k.final.no.umap <- pbmc3k.final
pbmc3k.final.no.umap[["umap"]] <- NULL
DimPlot(pbmc3k.final.no.umap) + RotatedAxis() + FontSize(x.title = 15, y.title = 15) + NoLegend() + labs(title = "Clustering of 2.700 PBMCs") 
```
...
```{r}
#Make the selected cells into mini cluster 
Idents(pbmc3k.final, cells = select.cells) <- "NewCells"
#Assign markers back specific to the new cells 
...
```
```{r}
#Combineplot function 
plot1 <- DimPlot(pbmc3k.final)
#Create scatter plot with Pearson correlation value
